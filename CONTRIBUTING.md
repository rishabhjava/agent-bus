...
- Before pushing: `npm run check` (syntax), `npm test` (installer tests — hermetic, run against throwaway temp homes with stubbed host CLIs, never your real config), and `npm run smoke` (spawns the MCP server and exercises every read path against whatever agents exist on your machine — on a machine with no agents it should still list tools and return empty session lists).
