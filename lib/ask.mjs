...
async function askClaude({ prompt, session_id, cwd, model, timeoutMs }) {
  ...
  if (res.timedOut) throw new Error(`claude timed out after ${timeoutMs / 1000}s (bus-side timeout_s)`);
  let parsed;
  try {
...
async function askCodex({ prompt, session_id, cwd, allow_writes, timeoutMs }) {
  ...
  if (res.timedOut) throw new Error(`codex timed out after ${timeoutMs / 1000}s (bus-side timeout_s)`);
...
export function resolveTimeoutMs(timeout_s) {
  if (timeout_s === 0) return 0;
  return Math.max(timeout_s ?? 240, 30) * 1000;
}
...
export async function askAgent({
  ...
  const timeoutMs = resolveTimeoutMs(timeout_s);
